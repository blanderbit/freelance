 ...
 * @param bool $generate_unique_name
 * @return bool
 */
function upload_files($path, $filename)
{
    if(isset($filename['name'])){
        ...
        if ($filename['size'] <= 20 * $megabyte){
            if((isset($_POST['user_uid'])) and (isset($_POST['user_id'])) and (isset($_POST['education-certificats-type']))){
                ...
                return move_uploaded_file($filename['tmp_name'], $target_file);
            }
        }
